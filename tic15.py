...
class Learner:
    ...
    def nn_test(s, X): #does it use the latest params?
...
class Selfplay:
    ...
    def play(s):
        ...
        while True: # Update states of both players 
            ...
            s.tries = np.empty((0,dim*3*dim*3))
            ...
            for b in [(0,0),(0,1),(0,2),(1,0),(1,1),(1,2),(2,0),(2,1),(2,2)]:
                if b not in s.learner.enum_actions(s.state):
                    ...
                    for i in random.sample(range(0,4), j):
                        ...
                        s.X = np.append(s.X, [s.learner.fullimg(rot1).flatten()], axis=0)
                        s.y = np.append(s.y, [0.0], axis=0)
                    s.state[b] = init
            ...
            for i in range(0,4):
                ...
                s.X = np.append(s.X, [s.learner.fullimg(rot2).flatten()], axis=0)
            ...
            if s.state.full() or s.state.won(1) or s.state.won(2):
                ...
                s.y = np.concatenate((s.y, s.y_game), axis=0)
                s.y_game = np.empty((0))
                s.moves = 1
            else:
                s.moves += 1
