...
#ALTERNATIVES (go back to tic11)

#elements in all and not in enumstate +rotations get y=0 

#keep working like tic11. But also run in parallel predictions over range(0,9), all max state superior to (if too hard to code, if  maxdifferent than of enumaction pred), assign y=0 to this imgstate
...
class Learner:
    def __init__(s, player, alpha = None, epsilon = None):
        ...
        s.gamehist = []
        s.traced = False
        s.all = [(0,0),(0,1),(0,2),(1,0),(1,1),(1,2),(2,0),(2,1),(2,2)]
        s.nn = Regressor(
            layers=[
...
class Selfplay:
    ...
    def __init__(s, learner = None, other = None):
        ...
        s.gamma = 0.7
        s.all = [(0,0),(0,1),(0,2),(1,0),(1,1),(1,2),(2,0),(2,1),(2,2)]
        #s.idall = [np.array(range(0,9))][0]

    def reset(s):
        s.state = State()
        s.learner.reset()
        s.other.reset()
        s.tempstates = s.state
        s.wrongstate = s.state
    ...
    def play(s):
        ...
        while True: # Update states of both players 
            ...
            s.tempstates = s.state
            for a in s.all:
                b = s.state[a] #for reinitilization
                s.state[a] = 2
                s.tempstates = np.vstack((s.tempstates, s.state))
            ...
            try:
                s.predvec = s.learner.nn_test(s.tries)
                global max
                max = s.predvec.argmax()
                global pred
                pred = s.predvec.max()
            except:
                global max
            ...
            #LEARN TO AVOID ILLEGAL MOVES
            s.tempstates = np.split(np.delete(s.tempstates, range(0,3), 0), 9, 0)
            remaining = np.empty([1,])
            for i in range(0, 9):
                if (s.tempstates[i]==2).sum() != (s.tempstates[i]==1).sum():
                    remaining = np.vstack((remaining, int(i)))
            ...
            global maxchanged
            maxchanged = 'no'
            while max not in remaining:
                print 'TEST', max, remaining
                s.wrongstate = [s.tempstates[max]]
                ...
                if s.X.shape[0] == 1:
                    ...
                    for i in range(0,4):
                        rot = np.rot90(s.wrongstate[0], i)
                        s.X = np.vstack((s.X, s.learner.fullimg(rot).flatten()))
                        s.y = np.vstack((s.y, 0.0))
                    rankmax += 1

                try: #take next max in predictions
                    max = np.argsort(s.predvec)[rankmax][0]
                    print 'maxchanged with pred'
                except:
                    s.idall = [item for item in s.idall if item != max]
                    max = random.sample(s.idall, 1)[0]
                    print 'maxchanged no pred'
            ...
            #ONCE LEARNER DID A LEGAL MOVE, LEARN X and y and append
