...
class Selfplay:
    ...
    def play(s):
        ...
        while True: # Update states of both players 
            ...
            #VALUE FOR ILLEGAL MOVES:
            #   'illegal state' has more O than X
            #   set its value to 0

            #if max not in s.learner.enum_actions(s.state):
            #   for i in range(0,4):
            ...
            if s.games %1 == 0:
                if s.state.full() or s.state.won(1) or s.state.won(2):
                    np.savetxt('/Users/Thomas/Dropbox/DS/thesis/data.txt', np.hstack((s.X, s.y)))
...
g.selfplay(10)
print 'wins proportion: ', s.selfplay.wins/s.selfplay.games
