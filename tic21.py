...
os.environ["THEANO_FLAGS"] = "mode=FAST_RUN,device=cpu,exception_verbosity=high"
import theano
...
class Learner:
    ...
    def nn_train(s, X, y):
        ...
        if os.path.exists('/Users/Thomas/git/thesis/nn.pkl'):
            nn_load = pickle.load(open('nn.pkl', 'rb'))
            nn_load.fit(X, y)
            params = nn_load.get_parameters()
        else:
            params = None
    ...
    def next_action(s, state): #decide action after maximizing gain
        ...
        #the random factor will be proportional to the size of the training data
...
class Game:
    ...
    def __init__(s, learner = None , other = None ):
        if learner == None: s.learner = Learner(player=2) #define if we want a second player 
        else: s.learner = learner
        if other == None: s.other = Learner(player=1)
        else: s.other = other
...
class Selfplay:
    ...
    def play(s):
        ...
        while True: # Update states of both players 
            ...
            print 'learner randomness ', s.learner.epsilon, 'other randomness ', s.other.epsilon
            ...
            if s.games %(selfplays/10) == 0 or s.games == selfplays:
                if s.state.full() or s.state.won(1) or s.state.won(2):
                    ...
                    obs = [propwin, proploss]
                    print 'propwins, proploss: ', obs
