...
class Learner:
    ...
    def next_action(s, state): #decide action after maximizing gain
        ...
        #print s.enum_actions(state)[max]
    ...
    def next(s, state):
        ...
        if s.laststate_hash != None:
            ...
            #NEW FUNCTION
            s.valuefunc[s.laststate_hash] = s.alpha * (val - s.valuefunc[s.laststate_hash] + 1)
...
class Selfplay:
    ...
    def play(s):
        ...
        while True: # Update states of both players 
            ...
            #try/pass and if one solution left, return s.tries[0]
            try:
                global max
